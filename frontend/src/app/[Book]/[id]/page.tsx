...
const ChapterContent = () => {
  ...
  const bookTitle = decodeURIComponent(pathParts[0]).replace(/-/g, " ");
  const chapterTitle = decodeURIComponent(pathParts[1]).replace(/-/g, " ");
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        ...
        setContent({ chapter });
        setNextChapter(bookData.chapters[chapterIndex + 1] || null);
        setPreviousChapter(bookData.chapters[chapterIndex - 1] || null);
      } catch (error) {
      ...
      <div className="flex flex-col items-center min-h-screen bg-gray-900 text-gray-200">
        <div className="w-full md:max-w-4xl mx-auto bg-gray-800 text-white rounded-lg shadow-md p-6">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-2xl font-bold text-start md:text-center">{content.chapter.title}</h1>
            <select
            ...
            <div className="flex-1 text-left">
              {previousChapter && (
                <button
                  onClick={() =>
                    router.push(
                      `/${bookTitle.replace(/ /g, "-")}/${previousChapter.title.replace(/ /g, "-")}`
                    )
                  }
            ...
            <div className="flex-1 text-right">
              {nextChapter && (
                <button
                  onClick={() =>
                    router.push(
                      `/${bookTitle.replace(/ /g, "-")}/${nextChapter.title.replace(/ /g, "-")}`
                    )
                  }
                  className="bg-gradient-to-r from-gray-600 to-gray-700 text-white px-4 py-2 rounded hover:bg-gradient-to-l hover:from-gray-500 hover:to-gray-600 transition duration-200"
                >
